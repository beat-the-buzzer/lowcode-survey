<template>
  <div class="m-10px">
    <div :id="`${id}-inline`" v-html="contentValue" class="p-10px"></div>
...
const toolbarSimple =
  "fontsize forecolor backcolor bold italic fullscreenplus | alignleft aligncenter alignright";
const commInit = {
  cleanup: true,
  language: "zh_CN", // 语言类型
  font_size_formats: "12px 14px 16px 18px 20px 22px 24px 28px 32px 36px", // 字体大小
  line_height_formats: "0.5 0.8 1 1.2 1.5 1.75 2 2.5 3 4 5", // 行高配置，也可配置成"12px 14px 16px 20px"这种形式
  ...
  setup: (editor: any) => {
    ...
    editor.ui.registry.addButton("fullscreenplus", {
      ...
      onAction: (_) => {
        ...
        const inlineId = tinymce.activeEditor.id;
        tinymce.execCommand("mceRemoveControl", false, inlineId);
        nextTick(() => {
          console.log("==============");
          tinymce.init(Object.assign(commInit, defaultInit));
        });
      },
    });
  },
...
// onUnmounted(() => {
//   console.log("tinymceonUnmounted");
//   tinymce.remove(`#${props.id}-inline`);
// });
