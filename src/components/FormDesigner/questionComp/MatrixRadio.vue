  ...
  <div>
    <div class="table">
      <div class="table-th">
        ...
        <div
          v-for="(item, index) in data.attribute.vertical
            ? data.row
            : data.children"
          :key="item.id"
          class="table-td"
          @click.stop="
            !data.attribute.vertical &&
              handleOptionSetting(item, index + 1, 'matrix_radio_column')
          "
        >
          <InlineEditor
          ...
          />
          <el-dropdown
            class="absolute right-0 top-0 m-5px"
      ...
      >
        ...
        <div
          class="table-td"
          v-for="(col, i) in data.attribute.vertical ? data.row : data.children"
          :key="col.id"
        >
          <el-radio v-model="checked" size="large">
            <el-input
              size="small"
              v-if="
                data.children[data.attribute.vertical ? index : i].attribute
                  ?.addInput
              "
            ></el-input>
          </el-radio>
    ...
    <div v-if="activeKey === data.id" class="m-10px mt-20px">
      ...
      <el-button link type="primary" @click.stop="handleAddRow">
        <el-icon><Plus /></el-icon>
        添加行
      </el-button>
    </div>
...
const handleAddRow = () => {
  data.value.row.push({
    title: `矩阵行${data.value.row.length + 1}`,
    id: buildShortUUID(),
  });
};
const handleAddChild = () => {
  data.value.children.push({
    title: `选项${data.value.row.length + 1}`,
    id: buildShortUUID(),
    attribute: {},
  });
};
...
const handleRowCommand = (command: any) => {
  const { type, index, item } = command;
  if (type === "right") {
    // 下移
    data.value.row.splice(index + 2, 0, item);
    data.value.row.splice(index, 1);
  }
  if (type === "left") {
    // 上移
    data.value.row.splice(index - 1, 0, item);
    data.value.row.splice(index + 1, 1);
  }
  if (type === "del") {
    // 删除
    if (data.value.row.length > 1) {
      data.value.row.splice(index, 1);
    } else {
      ElMessage.error("请保留最少一行");
    }
  }
};
