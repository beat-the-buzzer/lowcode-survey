...
<template>
  <div :class="`question-${index + 1} question-${data.id}`">
    ...
    <VideoUpload
      v-if="data.attribute?.addVideo && !data.attribute?.videoId"
      :question="data"
    />
    <div
      class="py-10px px-20px h-450px relative"
      v-if="data.attribute?.addVideo && data.attribute?.videoId"
    >
      <VideoPlayer preview :video-id="data.attribute.videoId" />
      <el-popconfirm title="是否确认删除?" @confirm="onDeleteVideo">
        <template #reference>
          <el-icon
            class="!absolute right-5px top-0 cursor-pointer z-20"
            size="20"
            color="#f55"
          >
            <CircleCloseFilled />
    ...
    <div class="q-content px-20px py-10px">
      <component
        ...
        :is="dom[data.type]"
        :singleChioceList="singleChioceList"
        :multiChioceList="multiChioceList"
...
import { deleteVideo } from "@/api/file";
...
const onDeleteVideo = async () => {
  try {
    await deleteVideo({ id: data.value.attribute.videoId });
  } finally {
    data.value.attribute.videoId = "";
  }
};
