      ...
      >
        <div class="flex items-center w-full">
          <div
            ...
            :class="{
              active: !readOnly && checkedIndex(item.id) > -1,
              disabled: readOnly,
            }"
            @click.stop="
              !readOnly && handleClick(item, item.attribute.exclusion)
            "
          >
            {{ checkedIndex(item.id) > -1 ? checkedIndex(item.id) + 1 : "" }}
          </div>
          <div class="flex-1">
            <div class="inline-block mr-20px" v-html="item.title"></div>
            <el-input
              class="!inline-block !w-150px"
              v-if="item.attribute.addInput && checkedIndex(item.id) > -1"
              :disabled="readOnly"
              v-model="checked[index].addition"
              size="small"
              @input="handleInput"
            >
